...
boot_alloc(uint32_t n)
{
	...
	if (!nextfree) {
		...
		// cprintf("neirong1 %08x\n",nextfree);
		// cprintf("neirong2 %08x\n",*nextfree);

		//               cprintf("neirong3 %s\n",nextfree);
		//  cprintf("hello!111111");
	}
	...
	//if(nextfree==0)
	// cprintf("\n\nmapping physical pages:  %x\n", (uint32_t)nextfree); 
	//  cprintf("neirong4 %08x\n",*nextfree);
...
mem_init(void)
{
	...
	// Your code goes here:
//	cprintf("the size of struct page is %d", sizeof(struct Page));
	...
	//  page2pa(pages);
	//cprintf("the size of struct page is %d", sizeof(struct Page));
//panic("the func is not finished\n");
	




	char *pg_va = (char *)pages;
    char *pgva_end = (char *)(pages + npages);
    //cprintf("pages start at %08x, end at %08x, offset is %d, npages is %d\n", 
     //   pg_va, pgva_end, (pgva_end - pg_va)/sizeof(struct Page), npages);
    char *map_va = (char *)UPAGES;
    //cprintf("map_va is %08x\n", map_va);
    for(; pg_va < pgva_end; pg_va += PGSIZE, map_va += PGSIZE){
        page_insert(kern_pgdir, pa2page(PADDR(pg_va)), map_va, PTE_U | PTE_P);
}
//////////////////////////////////////////////////////////////////////
	...
	// Your code goes here:
panic("the func is not finished\n");
	 pg_va = (char *)bootstack;
    pgva_end = (char *)(bootstack + KSTKSIZE);
    map_va = (char *)(KSTACKTOP - KSTKSIZE);
    for(; pg_va < pgva_end; pg_va += PGSIZE, map_va += PGSIZE){
        page_insert(kern_pgdir, pa2page(PADDR(pg_va)), map_va, PTE_W | PTE_P);
}
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	//int pp = ROUNDUP(0xffffffff - KERNBASE,PGSIZE);
	//for(i=0;i<pp;i=i+PGSIZE)
		//page_insert( kern_pgdir,(pa2page)(PADDR(0)) ,(void *)(KERNBASE+i),  PTE_P);

//panic("the func is not finished\n");

char *pg_pa = (char *)0x0;
    // number of pages need to map to va
    int map_npgs = ((uint32_t)0x10000000) >> PGSHIFT;
    map_va = (char *)KERNBASE;
    int i = 0;
    for(; i < map_npgs; ++i){
        if(i >= npages && i % npages == 0){
            //cprintf("i is equal to npages - 1\n");
            pg_pa = (char *)0x0;
        }
        page_insert(kern_pgdir, pa2page((physaddr_t)pg_pa), map_va, PTE_W | PTE_P);
        // all of the pages are mapped, return to 0x0 physical page
        map_va += PGSIZE;
        pg_pa += PGSIZE;
    }
    // after insert free page -> pp_ref is not equal to 0 
    struct Page *free_tmp = chunk_list;
    while(free_tmp){
        free_tmp->pp_ref = 0;
        free_tmp = free_tmp->pp_link;
    }
    free_tmp = page_free_list;
    while(free_tmp){
        free_tmp->pp_ref = 0;
        free_tmp = free_tmp->pp_link;
}
...
page_alloc(int alloc_flags)
{
	if(!page_free_list){
		...
		struct Page* pagen = page_free_list;
		page_free_list = page_free_list->pp_link;
				
		if (alloc_flags & ALLOC_ZERO){
			//char* start = page2kva(pagen);
			memset(page2kva(pagen), 0, PGSIZE);
		}
		
		pagen->pp_link = NULL;
	//	cprintf("pagen %0x8, pagen+1  %0x8, pagen-> %0x8\n", *pagen,*(pagen+1), *(pagen->pp_link));
		pagen->pp_ref = 0;
		return pagen;
	}
...
if(n == 1){
	page_alloc(alloc_flags);
}
struct  Page *phead = page_free_list;
struct Page *ptail = page_free_list+1;
int m = 1;

while( phead != NULL){
	if( phead - 1 == phead ->pp_link){
		m++;
		phead = phead ->pp_link;
...
//int i = 0;
//struct  Page* paaa = point;
//for(; paaa != pointN && pbbb!= pointN ;){
//	cprintf("hhhhhthe address of each page_free_list %08x\n",paaa );
//	paaa=paaa->pp_link;
//	cprintf("the address of each page_free_list %08x\n",pbbb );
//	pbbb = pbbb-1;	
	
//}

//cprintf("there is %d pages\n", m);

if(m == n){
 	if(page_free_list == ptail-1){
		page_free_list = phead->pp_link;
	}else{

		ptail->pp_link= phead ->pp_link;
	}

	for(i=0 ; i <n ;i++){
		if (alloc_flags & ALLOC_ZERO){
			memset(page2kva(phead+i), 0, PGSIZE);
		}
		(phead+i)->pp_link=phead+(i+1);
	}
	
		(phead+(n-1))->pp_link == NULL;
...
/*
	...
	cprintf("the address of head %08x\n",head);
	cprintf("the address of tail %08x\n",tail);

	while(head != NULL){
		if (head - head->pp_link == 1)
			now++;
		else{
			tail = head;
			now = 1;
		...
		if (now == n){
			if (tail == page_free_list + 1)
				page_free_list = head->pp_link;
			else
				tail->pp_link = head->pp_link;

			int idx;
			for(idx = 0; idx < n; idx++){
				(head + idx)->pp_link = head + (idx + 1);
				if (alloc_flags & ALLOC_ZERO){
					char* start = page2kva(head + idx);
					memset(start, 0, PGSIZE);
				}
			}
			(head + (n - 1))->pp_link = NULL;
...
/*	struct Page * cpages = pp;
	int i = 0;
	for()*/
	int idx;
	for(idx = 0; idx < n-1; idx++)
		if ((pp + idx) -> pp_link - (pp + idx) != 1)
			return -1;
	for(idx = 1; idx < n; idx++)
		(pp + idx) -> pp_link = (pp + (idx - 1));

	struct Page *head = page_free_list;
	struct Page *tail = NULL;
	while (pp < head && head != NULL){
		tail = head;
		head = head -> pp_link;
	}
...
/*	struct Page* prev = page_free_list;
	...
	while((uint32_t)pp < (uint32_t)prev && prev != NULL){
		next = prev;
		prev = prev->pp_link;
	}
...
*/
	...
	struct Page *ph =page_free_list;
	struct Page *iser = NULL;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (pgdir[pde_idx] == 0){
		...
		if (page == 0)
			...
			page->pp_ref += 1;
			pgdir[pde_idx] = page2pa(page) | PTE_P | PTE_W;
			return (pte_t *) (PTE_ADDR(page2kva(page))) + pte_idx;
		}
	} else{
		return (pte_t *) KADDR(PTE_ADDR(pgdir[pde_idx])) + pte_idx;
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	 pte_t* pte = NULL;
   	...
   	int idx, num = size / PGSIZE;
	for(idx = 0; idx < num; idx++){
		pte_t *pte = pgdir_walk(pgdir, (void *)(va + idx * PGSIZE), 1);
		*pte = pa + idx * PGSIZE;
	}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	pte_t *pt_start;
	struct Page* old = page_lookup(pgdir, va, &pt_start);
	
	if (old != NULL){
		if (old == pp){
			int pd_idx = PDX(va);
			// Page Directory Entry Permision
			pgdir[pd_idx] |= (perm | PTE_P | PTE_W);
			// Page Table Entry Permision
			*pt_start &= ~0xfff;
			*pt_start |= (perm | PTE_P | PTE_W);
			return 0;
		}
		page_remove(pgdir, va);
	}

	pt_start = pgdir_walk(pgdir, va, 1);
	
	if (pt_start == NULL)
		return -E_NO_MEM;
	else{
		int pte_idx = PTX(va);
		*pt_start = page2pa(pp) | (perm | PTE_P | PTE_W);
...
check_page_alloc(void)
{
	...
	for (pp = page_free_list; pp; pp = pp->pp_link)
		--nfree;
//	cprintf("thennumber of nfree is %d",nfree);
	//`assert(nfree == 0);
...
//		cprintf("the page number is %08x,pagesize is %08x",page2pa(tmp),PGSIZE);	
	for( tmp = pp, i = 0; i < num_page - 1; tmp = tmp->pp_link, i++ )
	{
		if(tmp == NULL) 
		{
